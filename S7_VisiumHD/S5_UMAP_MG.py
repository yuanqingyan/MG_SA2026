...
sc.pp.highly_variable_genes(adata6, 
                            n_top_genes=2000, 
                            flavor='seurat')

coord_keys = ('x', 'y', 'spatial_cropped150_buffer') 

adata6.obs['x']=adata6.obsm['spatial_cropped150_buffer'][:,0]
adata6.obs['y']=adata6.obsm['spatial_cropped150_buffer'][:,1]
...
adata6.obs['NewCT'] = adata6.obs['NewCT'].astype('category')
...
banksy_dict, banksy_matrix = generate_banksy_matrix(adata6,
                                                    banksy_dict,
                                                    lambda_list,
                                                    max_m)
...
banksy_dict["nonspatial"] = {
  0.0: {"adata": concatenate_all([adata6.X], 0, adata=adata6), }
}
...
banksy_adata.write(f"{SaveFolderPath}/UMAP_for_thymus6_fromBanksy.h5ad")


#####################
banksy_adata=sc.read_h5ad(f"{SaveFolderPath}/UMAP_for_thymus6_fromBanksy.h5ad")
rename_dict2 = {
  "DPT":"T/NK",
  "CD4.Naive":"T/NK",
  "mcTEC":"Ep",
...
plt.savefig(f"{FigOut}/umap_MG_label.pdf")
...
sc.pl.umap(banksy_adata, 
           ...
           legend_fontsize='small',
           legend_fontweight='light')
plt.savefig(f"{FigOut}/umap_MG_noLabel.tiff")
...
sc.pl.umap(banksy_adata, 
           ...
           vmax=1.5,
           vmin=0.2)
plt.savefig(f"{FigOut}/PaperGene_MG.pdf")
...
sc.pl.umap(banksy_adata, 
           ...
           vmax=1.5,
           vmin=0.2)
plt.savefig(f"{FigOut}/PaperGene_MG.tiff")
