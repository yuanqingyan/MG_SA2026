...
allCT<-list.files(dataFolder,pattern="sampleMeta_")
CT<-gsub("sampleMeta_","",gsub(".csv","",allCT))
...
allT_DEG<-lapply(1:length(CT),function(iC){
  ...
  pT<-pdata[pdata$Dis %in% c("MG",'NoMG') & pdata$Study %in% 'NU' & pdata$n_cells>=40,]
...
saveRDS(allT_DEG,file=sprintf("%s/pseudoBulk_allT_DEG.RDS",dataFolder))
